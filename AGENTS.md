...
- 古典派を既定とし、対象リポジトリが制御できる協働先は実物を使う。どの境界を差し替えてよいかの具体は対象リポジトリの実装言語のテスト規約が持ち、このpackageは所在を戦略へ書くだけで一覧を再定義しない。BDD IDの文法とテストへの転記も、それぞれの持ち主の規約に従い、このpackageでは定めない。
