...
- 古典派を既定とし、対象リポジトリが制御できる協働先は実物を使う。テストで差し替えてよい境界（制御できない外部の境界、時計、採番）の言語に依らない原則は、このpackageの`design-test-strategy`の`references/test-strategy-judgment.md`が一か所で持つ。差し替えの仕組み（gomock、MSWなど）は実装言語のテスト規約が持つ。BDD IDの文法とテストへの転記も、それぞれの持ち主の規約に従い、このpackageでは定めない。
- CIはローカルの完了判定と同じ検査の集合を走らせることを、戦略の判断として所有する。
