...
`design-test-strategy` は、品質リスクをどのテストレベルとサイズ（Small・Medium・Large）で確かめ、どこへ重複させないかという配分を持つ。CIがローカルの完了判定と同じ検査の集合を走らせること、固定の時間ではなく観測できる条件を期限付きで待つという言語に依らない原則も、ここにある。
