...
`design-test-strategy` は、品質リスクをどのテストレベルとサイズ（Small・Medium・Large）で確かめ、どこへ重複させないかという配分を持つ。CIがローカルの完了判定と同じ検査の集合を走らせること、固定の時間ではなく観測できる条件を期限付きで待つという言語に依らない原則、異常系をリスクに含めること、タイミングに依るテストを作らないこと、テストのために実装を曲げないこと、テストとテスト支援の部品を業務の言葉で名付けることも、ここにある。

`design-e2e-test-scenarios` は、E2Eへ残す経路と下位へ移す欠陥、外部サービスの公式テスト経路と共有の外部環境の扱いを持つ。`design-api-test-scenarios` は境界シナリオを、`plan-exploratory-testing` は探索的テストの計画を持つ。

どの境界をテストで差し替えてよいかの具体、障害の起こし方、BDD IDの文法とテストへの転記は持たず、それぞれの持ち主の規約（実装言語のテスト規約、業務知識の規約）に従う。
